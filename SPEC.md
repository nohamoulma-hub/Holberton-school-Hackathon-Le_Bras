...
Quand un nouveau stagiaire arrive, l'équipe doit accomplir manuellement une série de tâches dispersées sur plusieurs outils (créer un ticket d'onboarding, prévenir l'équipe, enregistrer la fiche stagiaire, préparer un document d'accueil, poser une réunion de bienvenue), sans processus unifié. Résultat : oublis, doublons, tâches faites en retard ou pas faites du tout, et une première semaine bâclée pour le nouvel arrivant. On propose un agent qui transforme une intention en langage naturel en un plan d'actions concrètes touchant plusieurs systèmes, soumet ce plan à validation humaine **action par action**, puis n'exécute que ce qui a été approuvé. Chaque exécution est journalisée de façon idempotente et annulable, pour qu'aucune action à effet de bord ne parte sans accord humain explicite et traçable.

## User stories (3 max)

1. **En tant que** responsable d'équipe, **je veux** décrire en une phrase l'arrivée d'un stagiaire **pour que** l'agent me propose un plan d'actions concret sans rien exécuter immédiatement.
...
flowchart LR
    ...
    EXEC --> T3[Tool: write_onboarding_record]
    EXEC --> T4[Tool: generate_welcome_doc]
...
- **Back** : API qui orchestre agent → plan → validation → exécution, ne fait jamais confiance au front pour l'exécution réelle.
- **Agent** : appelle le LLM avec la liste des tools disponibles, reçoit un plan structuré (pas d'exécution directe).
- **Outils** : chacun est un adaptateur avec effet de bord isolé, appelé uniquement par l'Executor après validation.
- **Stockage** : SQLite avec 3 tables clés — `plans`, `actions` (status: pending/approved/rejected/executed/cancelled + idempotency_key unique), `audit_log`.
...
| `write_onboarding_record` | `write_onboarding_record(intern_name: str, start_date: date, team: str, buddy: str) -> RecordId` | **Oui** — insertion dans la table `onboarding` |
| `generate_welcome_doc` | `generate_welcome_doc(intern_name: str, team: str, start_date: date) -> FilePath` | **Oui** — génération et sauvegarde d'un fichier markdown sur disque |
...
2. L'agent construit un plan de 5 actions (issue onboarding, message à l'équipe, fiche onboarding en base, doc de bienvenue, event calendrier) et l'affiche avec, pour chaque action, le détail de son effet de bord.
...
- **Toi** : back (API orchestrateur), agent/planner (appel LLM + tool calling), idempotence, journal d'audit + annulation.
- **Jo** : front (UI d'approbation action par action), les 5 adaptateurs Tool (issue/message/DB/fichier/calendrier), rédaction README/AGENTS.md.
...
Justification : c'est le genre de trou qu'on ne voit qu'en prod, jamais en demo — un plan à moitié validé par deux clics simultanés casserait justement le cœur technique du sujet (human-in-the-loop fiable). L'assumer explicitement montre qu'on a pensé au risque sans sur-ingénierer une solution qu'on n'a pas le temps de livrer en 3 jours.
